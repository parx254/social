...
*/
function deleteInboxMessage($messageID): bool {
$con = db();
$user = $_SESSION['username'] ?? null;
if (!$user) {
error_log("deleteInboxMessage(): no session user.");
return false;
}
if (!filter_var($messageID, FILTER_VALIDATE_INT)) {
error_log("deleteInboxMessage(): invalid message ID.");
return false;
}
$stmt = $con->prepare("
UPDATE messages
...
$stmt->close();
if ($success) {
error_log("Inbox message $messageID deleted by $user");
}
return $success;
}
/**
...
*/
function deleteSentMessage($messageID): bool {
$con = db();
$user = $_SESSION['username'] ?? null;
if (!$user) {
error_log("deleteSentMessage(): no session user.");
return false;
}
if (!filter_var($messageID, FILTER_VALIDATE_INT)) {
error_log("deleteSentMessage(): invalid message ID.");
return false;
}
$stmt = $con->prepare("
UPDATE messages
...
$stmt->close();
if ($success) {
error_log("Sent message $messageID deleted by $user");
}
return $success;
}
/* --------------------------------------------------------------------------
...
if ($messageID && basename($_SERVER['PHP_SELF']) === 'inbox.php') {
...
$messageID = filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);
if ($messageID && basename($_SERVER['PHP_SELF']) === 'messages.php') {
...
$from    = htmlspecialchars($row['from_user']);
$message = nl2br(htmlspecialchars($row['message']));
echo "
<div class='message-card trash'>
  <div class='message-header'>
    <span class='from-user'>From $from</span>
  </div>
  <div class='message-body'>
    $message
  </div>
...
$isPhoto = ($type === 'photo');
$locationName = $isPhoto ? 'location' : 'videolocation';
$categoryName = $isPhoto ? 'category' : 'videocategory';
$textName     = 'blogtext';
$fileAccept   = $isPhoto ? 'image/*' : 'video/mp4,video/x-m4v,video/*';
// ACTION FLAG FOR control.php
$actionValue  = $isPhoto ? 'add_photo' : 'add_video';
$icon  = $isPhoto ? 'fa-file-image-o' : 'fa-file-video-o';
$label = $isPhoto ? 'Post Photo' : 'Post Video';
echo "
<div class=\"post$type\">
  <h3>$label</h3><br>
  <form id=\"postForm_$type\" action=\"control.php\" method=\"POST\" enctype=\"multipart/form-data\">
    ...
    <br><br>
    <p>
    <textarea name=\"$textName\" maxlength=\"90\" placeholder=\"Type here...\" required></textarea>
  </p>
  <br>
  <i class=\"far $icon\"></i>
  <input type=\"file\" name=\"Filename\" accept=\"$fileAccept\" required>
  <br><br>
...
<div class='editprofposts'>
  <form class='edit-post-form' method='POST'>
    <h3>Edit Post from <?= htmlspecialchars($location) ?> on <?= $date_posted ?></h3><br>
    <textarea name='blog' maxlength='160' rows='10' required><?= htmlspecialchars($blog) ?></textarea>
    <br>
    <i class='far fa-globe'></i>
...
$stmt = $con->prepare("
UPDATE profiles
SET Bio = ?, Birthday = ?, HomeCity = ?
WHERE userID = ?
");
if (!$stmt) {
echo "Error preparing update: " . htmlspecialchars($con->error);
return;
}
...
$stmt->close();
if (!$ok) {
echo "Error updating profile: " . htmlspecialchars($con->error);
return;
}
...
// 3. FIRST-TIME USER? (empty profile)
$isNewUser = empty($bio) && empty($birthday) && empty($city);
if ($isNewUser) {
echo "
<div class='results'>
</div>
<h2>Welcome!</h2>
<h5>Please fill out the following information to complete your profile.</h5>
<br>
<form action='control.php' method='POST'>
  <b>Bio:</b>
  <textarea name='blogtext' rows='10' placeholder='Biography' required></textarea><br><br>
  <b>Birthday:</b>
  <input type='date' name='birthday' required><br><br>
  <b>Home Town:</b>
  <input type='text' class='my-input' name='city' autocomplete='off' required><br><br>
  <center>
  <input type='submit' class='login-group submit' name='userinfo-submit' value='Submit'>
</center>
</form>
";
return;
...
<div id=\'changeHere\'>
  <form method=\'POST\' action=\'control.php\' enctype=\'multipart/form-data\'>
    <span class=\'edit-label\'>Change Profile Photo</span>
...
<form method=\'POST\' action=\'control.php\' enctype=\'multipart/form-data\'>
  <span class=\'edit-label\'>Change Cover Photo</span>
  <input type=\'file\' name=\'Filename\' accept=\'image/*\' required>
...
<div class='prof_actions'>
  ...
  <form action='messages.php'>
    <button type='submit' " . ($highlight === 'messages' ? "class='highlighted'" : "") . "><i class='far fa-message'></i></button>
  </form>
  <form action='edit-profile.php'>
    <button type='submit' " . ($highlight === 'edit' ? "class='highlighted'" : "") . "><i class='fa fa-pencil-square-o'></i></button>
  </form>
  <button id='changePic'><i class='far fa-user-edit'></i></button>
  <form action='control.php' method='POST'>
...
// Returns sanitized HTML with <br> for newlines
return nl2br(
