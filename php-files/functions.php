...
function renderLikeButton(int $postID, int $likes, bool $liked = false): void { $isLoggedIn = !empty($_SESSION['userID']); $iconClass = $liked ? 'fas fa-heart' : 'far fa-heart'; $btnClass = $liked ? 'like-button liked' : 'like-button'; echo " <button type='button' class='{$btnClass}' data-post-id='{$postID}' " . ($isLoggedIn ? '' : 'disabled') . " > <i class='{$iconClass}'></i> <span class='like-count'>{$likes}</span> </button>"; }
// ==========================
// LIKE A POST
// ==========================
function like() {
